...
function CreateSchedule() {
  ...
  const registerShift = () => {
    ...
    setRegisteredShifts((prev) => [
      ...prev,
      { shift: selectedShift, user: selectedUser },
    ]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(
      "📤 Données envoyées :",
      JSON.stringify(
        { shifts: formattedShifts, user: selectedUser?.id },
        null,
        2
      )
    );
            ...
            <div className="min-h-screen bg-white flex flex-col items-center justify-items-center">
              ...
              <div className="flex flex-col items-center">
                <label className="block text-sm font-medium text-gray-900">
                  Sélectionnez une date de départ ( un jeudi ){" "}
                </label>
                <DatePicker
                  selected={startDate}
                  ...
                  <ul className="list-disc pl-4">
                    {registeredShifts.map(({ shift, user }) => (
                      <li key={shift} className="text-gray-800">
                        {shift} → {user.name} (ID: {user.id}, StatusID:{" "}
                        {user.statusId})
                      </li>
                    ))}
                  </ul>
              ...
              <button
                ...
                className={`mt-6 p-2 text-white rounded-md ${
                  isReadyToSubmit
                    ? "bg-orange-500 hover:bg-indigo-500"
