...
export default function ShiftCard() {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-gray-200 overflow-hidden">
        ...
        <div className="h-[calc(100%-4rem)] p-4 space-y-6 overflow-auto snap-mandatory snap-y overscroll-contain">
          ...
          {availableShifts.length > 0 ? (
            availableShifts.map((shift, index) => (
              <ShiftCardItem key={index} shift={shift} status="Available" onTakeShift={setSelectedShift} />
            ))
          ) : (
      ...
      {selectedShift && <ShiftPopup shift={selectedShift} onClose={() => setSelectedShift(null)} />}
    </div>
  );
}
    ...
    <div className="border border-gray-300 p-4 rounded-lg shadow-sm bg-gray-100 relative snap-start">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <span className="text-sm font-bold px-2 py-1 rounded-full bg-green-200 text-green-700">{shift.score}</span>
      ...
      {status === "Available" && (
        <button
          className="w-full text-center py-2 bg-blue-500 text-white rounded-md shadow-sm hover:bg-blue-600 transition mt-2"
...
<div className="bg-white p-6 rounded-lg shadow-xl w-80 text-center">
        <h2 className="text-lg font-semibold">Voulez-vous prendre le switch de {shift.name} ?</h2>
        <div className="flex justify-between mt-4">
          <button className="bg-green-500 text-white px-4 py-2 rounded" onClick={() => {
            console.log("Switch confirmé pour", shift.name);
            onClose();
          }}>Confirmer</button>
          <button className="bg-red-500 text-white px-4 py-2 rounded" onClick={onClose}>Annuler</button>
        </div>
      </div>
    </div>
  );
